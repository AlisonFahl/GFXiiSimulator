...
Terrain::Terrain(float scale, float maximumHeight)
{
	m_scale = scale;

	auto heightMap = FreeImage_Load(FIF_BMP, "../asset/texture/heightmap.bmp");
	...
	for (int i = 0; i < height; i++)
	{
		...
		for (int j = 0; j < width; j++)
		{
			...
			float positionU = j;/// (width * 0.5f);
			float positionV = i;/// (height * 0.5f);
...
void Terrain::Render()
{
	...
	glUniform3f(uniform_specularLightColor, 0.1f, 0.3f, 0.1f);
...
float Terrain::GetHeight(glm::vec2 point)
{
